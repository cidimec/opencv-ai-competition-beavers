...
def create_pipeline():
    ...
    stereo.setLeftRightCheck(
        False)  # Computes and combines disparities in both L-R and R-L directions, and combine them
    ...
    def depth_thread(self):
        ...
        while self.running:
            ...
            for depthData in spatialData:
                z_dists.append(
                    [depthData.spatialCoordinates.x, depthData.spatialCoordinates.y, depthData.spatialCoordinates.z])
        ...
        while True:
            ...
            if len(self.bboxes):
                ...
                if len(self.bboxes) > 1:
                    self.Compliance_People = (self.Compliance_People / self.Npersonas) * 100
                else:
                    self.Compliance_People = 1
            ...
            if self.Tiempo_segundos != contador_segundos and self.Tiempo_segundos > contador_segundos:
                ...
                if self.Pi != contador_calculos:
                    ...
                    print(aux_100, "Pi:", round(self.Pi, 2), "N:", self.Npersonas, "Mask:", self.Compliance_mask,
                          "Dist:", self.Compliance_People * 100)
            ...
            cv2.putText(Data_image, f"Personas: {self.Npersonas} ", (2, Data_image.shape[0] - 60),
                        cv2.FONT_HERSHEY_TRIPLEX, 0.4, color)
            cv2.putText(Data_image, f"tiempo: {int(self.Tiempo_segundos)} min", (2, Data_image.shape[0] - 310),
                        cv2.FONT_HERSHEY_TRIPLEX, 0.4, color)
            cv2.putText(Data_image, f"DS: {round(self.Compliance_People * 100, 2)} %", (2, Data_image.shape[0] - 30),
                        cv2.FONT_HERSHEY_TRIPLEX, 0.4, color)

            cv2.line(self.Top_view_Image, (0, 195), (300, 0), (255, 255, 255), 5)
            cv2.line(self.Top_view_Image, (0, 195), (300, 390), (255, 255, 255), 5)
            cv2.circle(self.Top_view_Image, (196, 180), 10, (0, 0, 255), -1)  # test circle
            cv2.circle(self.Top_view_Image, (210, 270), 10, (0, 0, 255), -1)  # test circle
